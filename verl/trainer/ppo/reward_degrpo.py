...
def get_custom_reward_fn(config: DictConfig) -> Optional[RawRewardFn]:
    ...
    it with additional keyword arguments from the configuration, including split_char for DeGRPO.
    ...
    function_name = reward_fn_config.get("name", "compute_reward")  # 默认使用compute_reward
    ...
    # 为DeGRPO添加split_char
    reward_kwargs["split_char"] = config.algorithm.get("split_char", "|||")
    # tokenizer将在load_reward_manager中填充
...
def load_reward_manager(
    ...
    """
    # reward_manager_name = config.reward_model.get("reward_manager", "degrpo")
    reward_manager_name = "degrpo"
    ...
    if compute_score is None:
        ...
        # 为自定义reward函数添加tokenizer
        final_compute_score = partial(compute_score, tokenizer=tokenizer)
...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager) -> tuple[dict, dict[str, Any]]:
    """
    ...
    Returns:
        Tuple of reward result dictionary (containing reward_tensor, a_rewards, b_rewards, etc.) 
        and extra info dictionary.
    """
    try:
    ...
    return reward_result, reward_extra_infos_dict
