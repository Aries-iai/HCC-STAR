...
@register_adv_est(AdvantageEstimator.DEGRPO)
def compute_degrpo_outcome_advantage(
    token_level_rewards: torch.Tensor,  # (bs, response_length)，仅作placeholder
    response_mask: torch.Tensor,  # (bs, response_length)
    index: np.ndarray,  # group index (bs,)
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    a_mask: torch.Tensor = None,  # (bs, response_length)，a部分的mask
    b_mask: torch.Tensor = None,  # (bs, response_length)，b部分的mask
    a_rewards: torch.Tensor = None,  # (bs,)，a部分的sequence-level reward
    b_rewards: torch.Tensor = None,  # (bs,)，b部分的sequence-level reward
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute decoupled advantage for DeGRPO: separate relative advantages for a and b parts.
    Requires a_rewards and b_rewards from external reward functions.
    """
    if a_mask is None or b_mask is None or a_rewards is None or b_rewards is None:
        raise ValueError("a_mask, b_mask, a_rewards, and b_rewards must be provided for decoupling.")
    assert a_mask.shape == response_mask.shape and b_mask.shape == response_mask.shape
    assert a_rewards.shape == (response_mask.shape[0],) and b_rewards.shape == (response_mask.shape[0],)
    assert torch.all((a_mask + b_mask) <= response_mask), "Masks must be subsets of response_mask."

    with torch.no_grad():
        id2a_score = defaultdict(list)
        id2b_score = defaultdict(list)
        id2a_mean = {}
        id2a_std = {}
        id2b_mean = {}
        id2b_std = {}

        bsz = a_rewards.shape[0]
        for i in range(bsz):
            idx = index[i]
            id2a_score[idx].append(a_rewards[i])
            id2b_score[idx].append(b_rewards[i])

        for idx in id2a_score:
            if len(id2a_score[idx]) <= 1:
                id2a_mean[idx] = torch.tensor(0.0, device=a_rewards.device)
                id2a_std[idx] = torch.tensor(1.0, device=a_rewards.device)
                id2b_mean[idx] = torch.tensor(0.0, device=b_rewards.device)
                id2b_std[idx] = torch.tensor(1.0, device=b_rewards.device)
            else:
                a_tensor = torch.stack(id2a_score[idx])
                b_tensor = torch.stack(id2b_score[idx])
                id2a_mean[idx] = torch.mean(a_tensor)
                id2a_std[idx] = torch.std(a_tensor)
                id2b_mean[idx] = torch.mean(b_tensor)
                id2b_std[idx] = torch.std(b_tensor)

        a_adv = torch.zeros_like(a_rewards)
        b_adv = torch.zeros_like(b_rewards)
        for i in range(bsz):
            idx = index[i]
            if norm_adv_by_std_in_grpo:
                a_adv[i] = (a_rewards[i] - id2a_mean[idx]) / (id2a_std[idx] + epsilon)
                b_adv[i] = (b_rewards[i] - id2b_mean[idx]) / (id2b_std[idx] + epsilon)
            else:
                a_adv[i] = a_rewards[i] - id2a_mean[idx]
                b_adv[i] = b_rewards[i] - id2b_mean[idx]

        advantages = a_adv.unsqueeze(-1) * a_mask + b_adv.unsqueeze(-1) * b_mask
        returns = advantages  # GRPO中returns=advantages
...
@register_policy_loss("degrpo")
def compute_policy_loss_degrpo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    loss_agg_mode: str = "seq-mean-token-sum-norm",
    config: Optional[DictConfig | AlgoConfig] = None,
    ...
    """
    Compute the clipped policy objective for DeGRPO with weighted a/b parts.
    """
    assert config is not None and a_mask is not None and b_mask is not None
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    loss_a = agg_loss(pg_losses * a_mask, a_mask, loss_agg_mode)
    loss_b = agg_loss(pg_losses * b_mask, b_mask, loss_agg_mode)
    pg_loss = alpha * loss_a + (1 - alpha) * loss_b
