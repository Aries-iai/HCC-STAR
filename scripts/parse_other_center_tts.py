...
# def remove_outliers(values, threshold=2):
#     if len(values) <= 1:
#         return values
...
def process_llm_jsonl(jsonl_path: str, out_csv_path: str, uuid2source: dict = None):
    ...
    if uuid2source is not None:
        ...
        for src, dfg in df.groupby("source"):
            base = os.path.splitext(out_csv_path)[0]  # e.g., df_gpt-4o-..._multicenter
            p = f"{base}_{src}.csv"
...
if __name__ == "__main__":
    res_path = 'data/llm_infer_res/ours_other_center_tts.jsonl'

    # 外部 LLM 结果路径
    claude_jsonl = "data/llm_infer_res/claude_other_center.jsonl"
    ds_jsonl = "data/llm_infer_res/ds_other_center.jsonl"
    gemini_jsonl = "data/llm_infer_res/gemini_other_center.jsonl"
    gpt4o_jsonl = "data/llm_infer_res/gpt4o_other_center.jsonl"
    gpt5_jsonl = "data/llm_infer_res/gpt5_other_center.jsonl"
    ...
    for sample in data:
        ...
        bt = []
        threshold_bt = 0.5
        for tname in final_treatment_result:  # 按你原注释：对最后一个 treatment_res 收集
    ...
    save_jsonl(fres, "data/visualization/staging_survival_multicenter.jsonl")
    ...
    #         return
    #     for src, dfg in df.groupby("source"):
    #         out_path = f"data/visualization/{prefix}_{src}.csv"
