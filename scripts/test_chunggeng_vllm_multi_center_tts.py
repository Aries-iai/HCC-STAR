            ...
            for line in f:
                if line.strip():  # Skip empty lines
                    try:
                        ...
                        data.append({
                            "prompt": record["liver_cancer_concat_prompt"],  # Store as string for hashing
...
def generate_single_response(prompt: str, client: OpenAI, retries: int = 3) -> Dict[str, Any]:
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"Attempt {attempt + 1} failed for prompt... Error: {str(e)}")
        ...
        if os.path.exists(output_path):
            with open(output_path, 'r', encoding='utf-8') as f:
                for line in f:
                    try:
                        record = json.loads(line.strip())
                        prompt_content = record.get("liver_cancer_concat_prompt", "")
            ...
            with tqdm(total=len(remaining_data), desc="Writing results", unit="item") as pbar:
                ...
                for future in as_completed(future_to_item):
                    ...
                    try:
                        ...
                        logger.info(f"Saved response for prompt...")
...
if __name__ == "__main__":
    ...
    INPUT_FILE = 'HCC-ClinReasoner/data/test_other_center.jsonl'
